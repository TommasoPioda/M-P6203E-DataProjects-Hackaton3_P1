...
The combined features represent the **most complete data representation** in our study because they are able to capture multiple aspects of the bibliographic relationship simultaneously. By synthesizing structured metadata, topological network importance, and deep semantic relationships, this representation allows the model to reconcile general paper characteristics with local graph connectivity and thematic similarity. This multidimensional approach ensures that the model moves beyond simple metadata lookups to understand the broader context of why a specific research paper refers to another.

Among the architectures tested, **XGBoost on combined features** was the best predictive model, achieving the highest performances while maintaining an high degree of interpretability. This model's success hihglights the **effectiveness of gradient-boosted trees in managing different-field tabular data**. While the Transformer architectures showed significant potential for capturing complex, non-linear interactions, they required more intensive tuning to match the robustness of the tree-based approach. 
