...
def load_citation_dataframe(
    ...
    return load_parquet_chunks(path_template, parts=parts, start=start, end=end, columns=columns)
...
def load_clean_citation_dataframe(
    ...
    df = load_citation_dataframe(path_template, parts=parts, start=start, end=end)
    return clean_citation_dataframe(df, drop_columns=drop_columns, remove_empty_rows=remove_empty_rows)


def load_citation_dataframe_from_files(
    file_paths: Sequence[str | Path],
    columns: Sequence[str] | None = None,
) -> pd.DataFrame:
    return load_parquet_files(file_paths, columns=columns)


def load_clean_citation_dataframe_from_files(
    file_paths: Sequence[str | Path],
    drop_columns: Sequence[str] = ("issn", "isbn", "url", "doi", "venue"),
    remove_empty_rows: bool = True,
) -> pd.DataFrame:
    df = load_citation_dataframe_from_files(file_paths)
    return clean_citation_dataframe(df, drop_columns=drop_columns, remove_empty_rows=remove_empty_rows)
...
def build_positive_negative_pairs(
    ...
    df_pos = (
        pairs.merge(article_features, on="article_id", how="inner")
    ...
    def sample_negative_ref_id(true_refs: set[str], article_id: str) -> str:
        if include_self_negatives:
            forbidden = true_refs
        else:
            forbidden = true_refs | {article_id}

        if len(forbidden) >= len(all_ids):
            forbidden = true_refs
    ...
    # Sample negatives only for the valid positive pairs that survived the merge
    valid_pos_article_ids = df_pos["article_id"].to_numpy()
    for article_id in valid_pos_article_ids:
        true_refs = {str(item) for item in refs_by_article.get(article_id, [])}
        fake_ref_ids.append(sample_negative_ref_id(true_refs, article_id))

    neg_pairs = pd.DataFrame({"article_id": valid_pos_article_ids, "ref_id": fake_ref_ids})

    df_neg = (
        neg_pairs.merge(article_features, on="article_id", how="inner")
        .merge(ref_features, on="ref_id", how="inner")
    )
...
def build_vector_text_columns(df: pd.DataFrame, include_authors: bool = True) -> pd.DataFrame:
    ...
    def build_article_text(row: pd.Series) -> str:
        parts = [
            ...
            normalize_text(row.get("authors_article", "")),
        ]
        if include_authors:
    ...
    def build_ref_text(row: pd.Series) -> str:
        parts = [
            ...
            normalize_text(row.get("authors_ref", "")),
        ]
        if include_authors:
...
def balance_classes(
    ...
    target_size = int(class_counts.max()) if method == "oversample" else int(class_counts.min())

    balanced_parts: list[pd.DataFrame] = []
    for class_value, class_df in df.groupby(target_col, dropna=False):
        replace = method == "oversample" and len(class_df) < target_size
        sampled = class_df.sample(n=target_size, replace=replace, random_state=random_state)
